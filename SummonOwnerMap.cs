...
/// NekoChan's). Bounded FIFO like <see cref="SeenSummonSet"/>, but cleared at a TRUE run/scene boundary only
/// (Plugin.RunBoundary.cs's <c>BankRunBoundary</c>) — NOT at the mid-run segment cut that also resets the
/// novelty set, because a pet alive across a boss cut must keep its owner mapping.
