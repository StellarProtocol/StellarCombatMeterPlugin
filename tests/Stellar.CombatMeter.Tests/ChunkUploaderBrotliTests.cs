...
public sealed class ChunkUploaderBrotliTests
{
    ...
    internal sealed class CapturingHandler : HttpMessageHandler
    {
        ...
        /// <summary>Zero-based call indices that raise a TRANSPORT exception instead of answering. The
        /// request is still RECORDED first, so a pin can assert what was on the wire when it blew up.</summary>
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (ThrowOnCalls.Contains(n)) throw new HttpRequestException($"fake transport fault on call {n}");
