...
public partial class AutoArchiveEngineTests
{
    ...
    // ---- config-independent stage-at-clear (vault-floor merge, run sea/ROjEKYLn01) ----
    // A multi-floor floor is its own run; the ~10 Hz sample catches its run-end transition at whichever
    // run-END state it lands on (a vault floor -> Settlement; End is skipped), and Settlement isn't
    // selected by default. A LATCHED clear (ClearedThisRun) must arm the stage archive on ANY run-end
    // transition REGARDLESS of selection, so the floor banks at its clear under its OWN run id before the
    // next floor's run id latches (else the late scene archive merges the two floors). The stage
    // checkboxes then only gate NON-clear stage cuts. Partial/fail runs (no clear) are unchanged.

    [Fact]
    public void Stage_latched_clear_arms_an_unselected_run_end_transition()
    {
        // Vault floor with LIVE stats: default {End} selected, but the transition is sampled at Settlement
        // (unselected). With the clear latched, the run-end transition arms + fires StageChange via the
        // normal HasStats path — so the floor banks its stats at the clear (under its own run id).
        var e = Armed(Live());                                    // default stage selection = {End}
        var s = Live() with
        {
            FlowStateVersion = 2, CurrentFlowState = DungeonFlowState.Settlement, ClearedThisRun = true,
        };
        Assert.Equal(ArchiveReason.StageChange, e.Evaluate(in s));
    }

    [Fact]
    public void Stage_unselected_run_end_transition_without_a_clear_still_does_not_fire()
    {
        // No behaviour change when there is NO clear: a run-end transition into an UNSELECTED stage
        // (Settlement, default {End}) with ClearedThisRun=false does not arm — the deterministic
        // 2026-07-30 stage-count behaviour is preserved for partial/fail runs.
        var e = Armed(Live());
        var s = Live() with
        {
            FlowStateVersion = 2, CurrentFlowState = DungeonFlowState.Settlement, ClearedThisRun = false,
        };
        Assert.Null(e.Evaluate(in s));
    }

    [Fact]
    public void Stage_latched_clear_does_not_arm_an_entry_side_transition()
    {
        // Safety (do NOT fire mid-run): the latched-clear arming is bounded to run-END stages
        // (IsRunEndState). A transition into an ENTRY-side state (Playing) must NOT arm even with a clear
        // latched, or a mid-run flow bump would cut a spurious archive of just the opener.
        var e = Armed(Live());
        var s = Live() with
        {
            FlowStateVersion = 2, CurrentFlowState = DungeonFlowState.Playing, ClearedThisRun = true,
        };
        Assert.Null(e.Evaluate(in s));
    }

    [Fact]
    public void Stage_after_bosskill_a_latched_clear_run_end_fires_the_empty_override()
    {
        // Boss dungeon SAFETY: the BossKill archive already banked the fight + reset stats. The run-end
        // transition (Settlement — unselected) with a latched clear then arms and fires the no-stats
        // fresh-clear override (line 289). At the plugin this StageChange has zero stats, so ManualArchive
        // skip-empties it (see Boss_run_banks_exactly_one_kill_entry_... below) because _clearMarkerBanked
        // is already set by the kill -> the boss run banks EXACTLY ONE kill entry, not a second.
        var e = new AutoArchiveEngine();
        Assert.Null(e.Evaluate(Live()));                          // adopt version 1
        e.OnArchived(200_000, ArchiveReason.BossKill);            // fight banked + stats reset
        var end = Live() with
        {
            NowMs = 201_000, FlowStateVersion = 2, CurrentFlowState = DungeonFlowState.Settlement,
            HasStats = false, HasFreshClear = true, ClearedThisRun = true,
        };
        Assert.Equal(ArchiveReason.StageChange, e.Evaluate(in end));
    }

    [Fact]
    public void Boss_run_banks_exactly_one_kill_entry_the_run_end_stage_skip_empties()
    {
        // The plugin half of the boss-no-double-bank safety: the BossKill archive banked the fight as a
        // kill -> _clearMarkerBanked = true. The subsequent run-end StageChange (armed by the latched-clear
        // rule, fired as the empty override above) has zero stats and MUST skip-empty, not bank a second
        // entry. Combined with the engine pin above, a boss run yields exactly one kill entry.
        Assert.False(Plugin.ShouldBankEmptyClearMarker(
            ArchiveReason.StageChange, "kill", alreadyBankedThisRun: true));
    }
