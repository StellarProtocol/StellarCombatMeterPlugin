...
public class RaidBossHpCaptureTests
{
    ...
    // Builds ResolveHpPair's bundled attr-map tuple. hp defaults to -1 (Plugin.ReplayHp.cs's ReadHpPair
    // convention for "AttrHp not present in the map"); the two MaxHp ids default to 0 (absent).
    private static (long Hp, long MaxHpBase, long MaxHpTotal) Attrs(long hp = -1, long maxHpBase = 0, long maxHpTotal = 0)
        => (hp, maxHpBase, maxHpTotal);

    // ── ResolveHpPair (spec item 2 / recon §5.2 + L4 + false-0%; I2 tier ④, full-chain review) ──────

    [Fact]
    public void ResolveHpPair_NativeTap_AlwaysWins_RepresentedAsPctOver100()
    {
        // Even when wire vitals AND the attr map disagree, the native tap is tier ① and wins outright.
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: true, nativePct: 37, Known(1, 2), Attrs(hp: 999, maxHpTotal: 999));
    ...
    public void ResolveHpPair_NativeZero_IsUsable_NotSentinel()
    {
        ...
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: true, nativePct: 0, Unknown, Attrs());
    ...
    {
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Known(40, 100), Attrs());
        Assert.Equal((40L, 100L, "vitals"), (hp, maxHp, src));
    }

    [Fact]
    public void ResolveHpPair_HasHpObservation_NeverFallsThroughToTierFour_EvenWhenAttrMapHasAFullPair()
    {
        // HasHpObservation=true but MaxHp never arrived on the row, and 11321 (tier ③) is absent too —
        // tier ④ (the local-player attr11310 fallback) is gated on `!HasHpObservation` and must NOT
        // fire here, even though the attr map happens to carry a full Hp+MaxHpBase pair.
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Known(25, 0), Attrs(hp: 999, maxHpBase: 999));
    ...
    public void ResolveHpPair_AttrFallback_PairsWithKnownHp_WhenWireMaxHpNeverArrived()
    {
        ...
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Known(25, 0), Attrs(maxHpTotal: 200));
    ...
    {
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Unknown, Attrs());
        Assert.Equal((0L, 0L, "none"), (hp, maxHp, src));
    }
    ...
    {
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Known(10, 0), Attrs());
        Assert.Equal((0L, 0L, "none"), (hp, maxHp, src));
    }

    // ── I2 (full-chain review): tier ④ — local-player enter-scene-self walk-in fallback ─────────────
    //
    // Unlike appear/delta (which route 11310/11320/11321 straight into vitals, dead per recon L4 —
    // see AttrHp's doc), the LOCAL PLAYER's third write path (enter-scene-self) has no dedicated
    // Hp/MaxHp branches and DOES store them in the generic attr map for that one entity — restoring
    // the walk-in HP fallback before the first vitals delta arrives. Reachable ONLY when the vitals
    // row has no Hp observation at all (`!HasHpObservation`), tiers ②/③ having already missed.

    [Fact]
    public void ResolveHpPair_AttrHpFallback_Fires_WhenNoVitalsHpObservationAtAll()
    {
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Unknown, Attrs(hp: 80, maxHpTotal: 100));
        Assert.Equal((80L, 100L, "attr11310"), (hp, maxHp, src));
    }

    [Fact]
    public void ResolveHpPair_AttrHpFallback_Prefers11321_Over11320()
    {
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Unknown, Attrs(hp: 80, maxHpBase: 90, maxHpTotal: 100));
        Assert.Equal((80L, 100L, "attr11310"), (hp, maxHp, src));   // 100 (total), not 90 (base)
    }

    [Fact]
    public void ResolveHpPair_AttrHpFallback_FallsBackTo11320_When11321Absent()
    {
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Unknown, Attrs(hp: 80, maxHpBase: 90));
        Assert.Equal((80L, 90L, "attr11310"), (hp, maxHp, src));
    }

    [Fact]
    public void ResolveHpPair_AttrHpFallback_MaxHpOnlyVitals_StillFallsThroughToTierFour()
    {
        // MaxHpOnly (HasHpObservation=false) — the false-0% shape — falls THROUGH tiers ②/③ (both
        // gated on HasHpObservation=true) to tier ④, and resolves there when the attr map
        // independently carries a full Hp+MaxHp pair.
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, MaxHpOnly(500), Attrs(hp: 12, maxHpTotal: 300));
        Assert.Equal((12L, 300L, "attr11310"), (hp, maxHp, src));
    }

    [Fact]
    public void ResolveHpPair_AttrHpFallback_Unusable_WhenAttrHpAbsent()
    {
        // HasHpObservation=false AND the attr map has NO Hp entry (only a MaxHp) — the false-0%
        // defect's core case, from EITHER source: a MaxHp-only signal must never mint a usable pair.
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, MaxHpOnly(500), Attrs(maxHpTotal: 500));
        Assert.Equal((0L, 0L, "none"), (hp, maxHp, src));
    }

    [Fact]
    public void ResolveHpPair_AttrHpFallback_Unusable_WhenNoMaxHpAnywhere()
    {
        var (hp, maxHp, src) = Plugin.ResolveHpPair(hasNativeBlood: false, nativePct: 0, Unknown, Attrs(hp: 80));
        Assert.Equal((0L, 0L, "none"), (hp, maxHp, src));
    }
